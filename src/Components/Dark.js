...
export default function Dark() {
    let [mystyle, newtext] = useState({
        color: 'white',
        backgroundColor: 'black'
    })
    let [enable, newenable] = useState("enable")
    const park = () => {
    ...
    return <div className='my-5' style={mystyle}>
        ...
        <div >
            <button type="button" class="btn btn-primary" onClick={park}>{enable}</button>
        </div>
    </div>;
}
